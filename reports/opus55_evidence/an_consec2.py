...
d=d[d['p']>=7]
p=d['p'].astype(np.int64); A=((d['a']>>5)&1).astype(np.int8)  # base 10 is index 5
Q=((d['q']>>5)&1).astype(np.int8)
o=d['o'].astype(np.float32); del d
print('n primes',len(p),'artin',A.sum(),A.mean())
x=A[:-1]; y=A[1:]; g=(p[1:]-p[:-1]).astype(np.int32)
N=len(x)
n11=int((x&y).sum(dtype=np.int64)); n10=int((x&(1-y)).sum()); n01=int(((1-x)&y).sum()); n00=N-n11-n10-n01
...
# omega corr
oa=o[:-1].astype(np.float64); ob=o[1:].astype(np.float64); print('r omega',((oa-oa.mean())*(ob-ob.mean())).mean()/(oa.std()*ob.std())); del oa,ob
# mod12 diag
r12=p%12
for r in [1,5,7,11]:
...
def ce(key,K):
    ...
    k2=key.astype(np.int64)*4+(x.astype(np.int64)*2+y); cnt=np.bincount(k2,minlength=K*4).reshape(K,4); del k2
