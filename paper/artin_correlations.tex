...
\begin{abstract}
	...
	$840$ accounts for $96.1\%$ and $97.8\%$ of the global deficit through joint-residue cell
	composition rather than within-cell association (an exact within/between decomposition). As an intermediate link we report the apparently unreported
	...
	joint-residue baseline, which itself carries $0.4195$ nats; under a held-out test it shows
	\emph{no} gain at all (both held-out comparisons are negative).
...
\begin{enumerate}
	...
	joint residues of $(p_n,p_{n+1})$ modulo $120$ and $840$ gives an exact within/between split
	of the global deficit in which $96.1\%$ and $97.8\%$ respectively is attributable to cell
	composition rather than within-cell association; the residual within-cell association
	($-0.000545$, $-0.000310$) is consistent with further small-modulus channels but has not
	...
	consecutive-prime correlation carries $0.000009$ nats of conditional-entropy reduction
	beyond the elementary joint-residue baseline and shows no held-out gain; both exclusion
	laws are consequences of character multiplicativity and hence computationally redundant;
...
descriptive reduction showing that, on the selected-cell metrics used here, $93$--$97\%$ and
$95\%$ respectively of both correlations is accounted for by the same elementary structure
...
of those correlations, which is nil (\S\ref{sec:content}). The exclusion laws are immediate
